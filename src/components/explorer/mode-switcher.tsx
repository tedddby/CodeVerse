...
export function ModeSwitcher({ className }: { className?: string }) {
  ...
  return (
    <div role="group" aria-label="Visual mode" className={cn("glass flex items-center gap-0.5 rounded-xl p-1", className)}>
          ...
          <span key={mode.id} className="group/tooltip relative inline-flex">
            <button
              ...
              className={cn(
                ...
                active ? "bg-signal/14 text-signal shadow-[inset_0_0_0_1px_rgba(77,226,255,0.35)]" : "text-ink-muted hover:bg-panel-raised hover:text-ink",
              )}
            >
            ...
            <span
              ...
              role="tooltip"
              className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 w-56 -translate-x-1/2 rounded-md border border-line-strong bg-panel-raised px-2.5 py-2 text-xs text-ink opacity-0 shadow-lg transition-opacity delay-300 duration-150 group-focus-within/tooltip:opacity-100 group-hover/tooltip:opacity-100"
            >
