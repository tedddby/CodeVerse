...
import {
  ...
  MIN_LOADING_MS,
  useExplorerPhase,
  type ExplorerPhaseInput,
} from "./use-explorer-phase";
...
describe("useExplorerPhase", () => {
  ...
  it("holds the loading screen for a minimum time, then plays the entrance transition", () => {
    const { result, rerender } = renderHook(
      (props: ExplorerPhaseInput) => useExplorerPhase(props),
      {
  ...
  it("skips the delays with reduced motion", () => {
    const { result } = renderHook(() =>
      useExplorerPhase({ status: "complete", worldReady: true, reducedMotion: true }),
    );
