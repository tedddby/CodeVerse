...
describe("TopBar", () => {
  it("links the repository safely and shows the analysed commit", () => {
    ...
    expect(screen.getByText(`@${mockRepositoryGraph.repository.commitSha.slice(0, 7)}`)).toBeInTheDocument();
  });
  ...
  it("switches modes through an accessible segmented control", async () => {
    ...
    expect(within(group).getByRole("button", { name: "Architecture mode (1)" })).toHaveAttribute("aria-pressed", "true");
    await user.click(within(group).getByRole("button", { name: "Complexity mode (5)" }));
    expect(state().visualMode).toBe("complexity");
    expect(within(group).getByRole("button", { name: "Complexity mode (5)" })).toHaveAttribute("aria-pressed", "true");
  });
  ...
  it("summarizes coverage and reveals details in a popover that Escape closes", async () => {
    ...
    const limited: RepositoryGraph = {
      ...
      analysis: {
        ...
        treeTruncated: true,
        warnings: [{ code: "LARGE_REPOSITORY", message: "Very large repository: only a sample of files was parsed." }],
      },
    };
    ...
    expect(within(details).getByText("Very large repository: only a sample of files was parsed.")).toBeInTheDocument();
...
describe("SelectionAnnouncer", () => {
  it("announces selection, focus and mode changes in polite live regions", () => {
    ...
    expect(screen.getByText("Selected file src/auth/auth.ts, TypeScript, 842 lines")).toBeInTheDocument();
