  ...
  return (
    <div>
      ...
      {stats.omittedFileCount > 0 ? (
        <div className="border-warn/30 bg-warn/5 text-ink-muted mt-3 rounded-lg border p-2.5 text-xs leading-relaxed">
          <Badge tone="warn">Partial</Badge> {pluralize(stats.omittedFileCount, "file")} in this
          directory {stats.omittedFileCount === 1 ? "was" : "were"} not included in the 3D world
          because of analysis limits. Counts above include them.
        </div>
      ) : null}
      ...
      <dl className="mt-3 grid grid-cols-4 gap-1.5">
        <StatTile
          label="Files"
          value={formatCompact(stats.fileCount)}
          hint={formatInteger(stats.fileCount)}
        />
        <StatTile
          label="Folders"
          value={formatCompact(stats.directDirectoryCount)}
          hint="Direct sub-directories"
        />
        <StatTile
          label="Symbols"
          value={formatCompact(stats.symbolCount)}
          hint={formatInteger(stats.symbolCount)}
        />
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        <ActionButton
          icon={<Crosshair />}
          onClick={() =>
            issueCameraCommand({ type: "focus-node", ref: { kind: "directory", id: directory.id } })
          }
        >
          Focus
        </ActionButton>
      ...
      {subdirectories.length > 0 ? (
        <PanelSection
          title={
            <Fragment>
              Sub-directories{" "}
              <span className="font-mono">{formatInteger(subdirectories.length)}</span>
            </Fragment>
          }
        >
          <ExpandableList
            ...
            renderItem={(child) => (
              <span className="flex min-w-0 items-center gap-2">
                <Folder aria-hidden="true" className="text-ink-subtle size-3.5 shrink-0" />
                <NodeLink
                  nodeRef={{ kind: "directory", id: child.id }}
                ...
                <span className="text-ink-subtle ml-auto shrink-0 font-mono text-[11px]">
                  {formatCompact(child.stats.fileCount)}
                </span>
              </span>
            )}
          />
            ...
            {largestFiles.map((file) => (
              <li key={file.id} className="flex min-w-0 items-center gap-2">
                ...
                <NodeLink
                  ...
                  title={file.path}
                >
                  {directory.path && file.path.startsWith(`${directory.path}/`)
                    ? file.path.slice(directory.path.length + 1)
                    : file.path}
                </NodeLink>
                <span className="text-ink-subtle ml-auto shrink-0 font-mono text-[11px]">
