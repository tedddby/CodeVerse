...
export const MOCK_REPOSITORY_SPEC: FixtureSpec = {
  ...
  files: [
    ...
    // src
    {
      path: "src/index.ts",
      lines: 96,
      imports: ["src/auth/middleware.ts", "src/payments/checkout.ts", "src/users/service.ts", "src/lib/config.ts", "src/api/routes.ts"],
      symbols: [
    ...
    },
    {
      ...
      lines: 388,
      imports: ["src/payments/stripe.ts", "src/users/service.ts", "src/payments/types.ts", "src/payments/pricing.ts"],
      symbols: [
    ...
    // tests
    { path: "tests/auth.test.ts", lines: 390, imports: ["src/auth/auth.ts", "src/auth/jwt.ts", "pkg:vitest"] },
    { path: "tests/payments.test.ts", lines: 280, imports: ["src/payments/checkout.ts", "pkg:vitest"] },
  ...
  commits: [
    { sha: "a1", message: "Harden JWT verification", author: "octo-ada", daysAgo: 2, files: ["src/auth/jwt.ts", "tests/auth.test.ts"] },
    { sha: "a2", message: "Add refund webhooks", author: "octo-bruno", daysAgo: 4, files: ["src/payments/stripe.ts", "src/payments/types.ts"] },
    { sha: "a3", message: "Session store uses transactions", author: "octo-ada", daysAgo: 9, files: ["src/auth/session.ts", "src/lib/db.ts"] },
    { sha: "a4", message: "CLI deploy status command", author: "octo-chen", daysAgo: 12, files: ["packages/cli/acme_cli/commands.py", "packages/cli/acme_cli/main.py"] },
    { sha: "a5", message: "Checkout pricing rules", author: "octo-bruno", daysAgo: 16, files: ["src/payments/pricing.ts", "src/payments/checkout.ts", "tests/payments.test.ts"] },
    { sha: "a6", message: "Billing invoice renderer", author: "octo-dara", daysAgo: 24, files: ["services/billing/invoice.go", "services/billing/main.go"] },
    { sha: "a7", message: "Route handlers for auth", author: "octo-ada", daysAgo: 31, files: ["src/api/handlers/auth.ts", "src/api/routes.ts"] },
    { sha: "a8", message: "User registration flow", author: "octo-chen", daysAgo: 38, files: ["src/users/service.ts", "src/users/repository.ts", "tests/users.test.ts"] },
    { sha: "a9", message: "SDK client retries", author: "octo-bruno", daysAgo: 45, files: ["packages/sdk/src/client.ts"] },
    { sha: "b1", message: "Architecture docs", author: "octo-dara", daysAgo: 60, files: ["docs/architecture.md", "README.md"] },
    { sha: "b2", message: "Structured logging", author: "octo-ada", daysAgo: 74, files: ["src/lib/logger.ts", "src/index.ts"] },
    { sha: "b3", message: "Hasher crate", author: "octo-chen", daysAgo: 90, files: ["services/hasher/src/lib.rs"] },
    { sha: "b4", message: "Report job scheduling", author: "octo-dara", daysAgo: 110, files: ["services/reports/src/main/java/com/acme/reports/ReportJob.java"] },
    { sha: "b5", message: "Config validation with zod", author: "octo-ada", daysAgo: 130, files: ["src/lib/config.ts", "package.json"] },
    { sha: "b6", message: "Initial payments module", author: "octo-bruno", daysAgo: 160, files: ["src/payments/stripe.ts", "src/payments/checkout.ts", "src/payments/types.ts"] },
    { sha: "b7", message: "Initial auth module", author: "octo-ada", daysAgo: 200, files: ["src/auth/auth.ts", "src/auth/jwt.ts", "src/auth/middleware.ts"] },
  ],
};
